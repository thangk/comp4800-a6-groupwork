...
class ToscrapeCrawlerSpider(Spider):
    ...
    allowed_domains = ["kapthang.dev"]
    start_urls = ["https://kapthang.dev"]
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.nodes_limit = -1  # Set to 100 for testing, or -1 for no limit
    ...
    def parse(self, response):
        if response.url not in self.url_to_id:
            ...
            self.id_counter += 1
            self.seen_edges = set()  # Add this line to initialize the set of seen edges
        ...
        for href in response.css("a::attr(href)").extract():
            ...
            if (source_id, target_id) not in self.seen_edges:
                self.seen_edges.add((source_id, target_id))  # Mark this edge as seen
